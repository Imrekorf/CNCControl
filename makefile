...
COMPILE_FLAGS = -std=c++17 -Wall -Wextra -ggdb3
...
INCLUDES := $(foreach dir, $(RINCDIRS), $(patsubst %, -I%, $(dir))) 

# General linker settings
ifeq ($(OS), Windows_NT)
	LINK_FLAGS = -lwiringPi -lpthread
else 
	LINK_FLAGS = -lwiringPi -lpthread
endif
...
define launchjson
{
    ...
    },
    {
        ...
        "preLaunchTask": "g++.exe build project"
    }
    ]
   }
endef
...
# Create directories for a generic project
.PHONY: init
init:
	...
	@mkdir -p .vscode
	@echo 	"$$maincpp" > src/main.cpp

	@echo 	"$$launchjson" > .vscode/launch.json

	@echo 	"$$tasksjson" > .vscode/tasks.json